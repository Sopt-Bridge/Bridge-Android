...
import kotlinx.android.synthetic.main.row_contents_library_simple.view.*

/**
 * Created by jihaeseong on 2018. 7. 3..
 */

class GroupContentsAdapter(internal var _context: Context) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    var items = ArrayList<Content>()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val convertView = LayoutInflater.from(_context).inflate(R.layout.row_contents_library_simple, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        with((holder as NowTrendViewHolder).itemView){
            ...
            if(items[position].contentsType==0){
                ...
                Glide.with(_context).load(ApplicationController.imageUrl(items[position].contentsIdx, 1)).override(UtilController.convertDpToPixel(120f, context).toInt(), UtilController.convertDpToPixel(78f, context).toInt()).into(contents_image_thumbnail)
            }else{
